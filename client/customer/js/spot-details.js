...
async function initializeSpotBookingPage(spotId) {
    ...
    const vehicleRegInput = document.getElementById('vehicle-reg-input'); // Nếu có
    ...
    payAndReserveBtn.addEventListener('click', async () => {
        ...
        try {
            payAndReserveBtn.disabled = true;
            payAndReserveBtn.textContent = 'Processing...';
            ...
            const bookingConfirmation = await response.json();
            alert('Booking successful!');
            window.location.href = 'my-bookings.html?bookingId=' + bookingConfirmation._id; 
