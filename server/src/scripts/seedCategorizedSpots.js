// File: server/src/scripts/seedCategorizedSpots.js
...
const seedCategorizedSpots = async () => {
    try {
        ...
        if (!owner) {
            console.log('No users found. Creating a default Host user...');
            ...
            console.log(`✅ Created default host user: ${owner.email}`);
        } else {
            console.log(`Using existing user: ${owner.email} as owner.`);
        }

        let count = 0;
        for (const spotData of categorizedSpots) {
            // Check if spot already exists (by name or address)
            ...
            if (!exists) {
                ...
                console.log(`Created spot: ${spotData.name} (${spotData.address})`);
                count++;
            } else {
                console.log(`Spot already exists: ${spotData.name}`);
            }
        }

        console.log(`🎉 Seeding completed. Added ${count} new categorized spots.`);
        process.exit(0);
    } catch (error) {
        console.error('❌ Error seeding categorized spots:', error);
        process.exit(1);
    }
};

seedCategorizedSpots();
